...
export default {
	...
	methods: {
		...
		onTouchstart(e) {
			...
			let expression = `(abs(x) > abs(y) * 1.7) ? min(max(${-this.buttonsWidth}, x), 0) : exit`
			// 尝试关闭其他打开的单元格
			this.parent && this.parent.closeOther(this)
			...
			this.panEvent = bindingX.bind({
				...
				// 及时释放BindingX,ios机造成外层可滚动组件事件阻塞
				this.panEvent = undefined
				this.moving = false
				if(res.state === 'exit') this.moveCellByAnimation('close')
				// ATTENTION: 安卓机上存在大量state=cancel的情况
				else {
					const {deltaX, deltaY} = res, elapse = Date.now() - startTime
				
					if(elapse <= 60 && (Math.abs(deltaX) > Math.abs(deltaY) * 1.7)) {
						// 左右滑动 60ms短触处理
						return this.moveCellByAnimation(deltaX > 0 ? 'close' : 'open')
					}
					...
					} else if((Math.abs(deltaX) > uni.$u.getPx(this.threshold)) && (Math.abs(deltaX) > Math.abs(deltaY) * 1.7)) {
