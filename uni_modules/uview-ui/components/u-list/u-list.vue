	...
	export default {
		...
		computed: {
			listStyle() {
				...
				if (!style.height) style.height = addUnit(this.sys.safeArea.height - 44, 'px')
		...
		methods: {
			...
			scrollIntoViewByAnchor(anchor) {
				// #ifdef APP-NVUE
				// 根据id参数，找到所有u-list-item中匹配的节点，再通过dom模块滚动到对应的位置
				const child = this.children.find(child => child.$refs[`u-list-item-${anchor}`] ? true : false)
				const ref = child ? child.$refs[`u-list-item-${anchor}`] : anchor
